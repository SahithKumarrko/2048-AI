...
class AI(object):
    ...
    def build_eval_monotone_table(self):
        ...
        for idx, row in enumerate(self.idx_to_row):
            row2 = tuple([0 if mrv==None else mrv for mrv in row])            
            L = R = 0
            m = 0
            for y in range3:
              if row2[y] and row2[y] >= row2[y+1]:
                m += 1
                L += m ** 2 * 4
              else:
                L -= abs((row2[y] or 0)- (row2[y+1] or 0)) * 1.5
                m = 0

            m = 0
            for y in range3:
              if row2[y] <= row2[y+1] and row2[y+1]:
                m += 1
                R += m ** 2 * 4
              else:
                R -= abs((row2[y] or 0)- (row2[y+1] or 0)) * 1.5
                m = 0
    ...
    def build_move_table(self):
        ...
        for idx, row in enumerate(self.idx_to_row):
            ...
            if max([0 if mrv==None else mrv for mrv in row_moved]) > max_cell:
